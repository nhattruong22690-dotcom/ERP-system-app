...
import { getCachedStats, syncCustomerStats, BranchStats } from '@/lib/services/customerStatsEngine';
...
const CustomerList: React.FC<CustomerListProps> = ({
    ...
    const fetchRemoteData = useCallback(async () => {
        ...
        setIsSearching(true);
        try {
            console.log('CustomerList: Fetching data with:', {
            ...
            if (isSearchActive) {
                // Sanitize term: escape single quotes and handle comma issue by quoting
                const safeTerm = searchTerm.trim().replace(/'/g, "''");
                query = query.or(`full_name.ilike."%${safeTerm}%",phone.ilike."%${safeTerm}%",email.ilike."%${safeTerm}%"`);
            ...
            if (err.name === 'AbortError') {
                console.log('CustomerList: Fetch aborted');
                return;
            }
            console.group('🔴 CustomerList: Fetch error details');

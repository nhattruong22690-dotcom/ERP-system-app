        ...
        >
            ...
            >
                ...
                <div className="flex-none relative h-fit bg-beige-soft/5">
                    ...
                    {/* Header Cover - Refined Height */}
                    <div className="relative h-28 sm:h-36 xl:h-44 overflow-hidden">
                        <div className="absolute inset-0 bg-[#0A0A0A]">
                            <div className="absolute inset-0 opacity-40 bg-[radial-gradient(circle_at_20%_30%,_#C5A059_0%,_transparent_50%),radial-gradient(circle_at_80%_70%,_#8E7341_0%,_transparent_50%)]"></div>
                            <div className="absolute inset-0 bg-noise opacity-20 mix-blend-overlay"></div>
                            <div className="absolute bottom-0 left-0 right-0 h-24 bg-gradient-to-t from-white to-transparent"></div>
                        </div>
                        <div className="absolute top-6 left-8 z-20">
                            <span className="bg-gold-muted/20 backdrop-blur-xl border border-gold-muted/30 text-gold-muted px-4 py-1.5 rounded-full text-[clamp(8px,0.7vw,10px)] font-black uppercase tracking-[0.2em] shadow-lg">
                                • Customer Dossier •
                            </span>
                        </div>
                    </div>

                    {/* Profile Overview Section - Sticky for visibility */}
                    <div className="px-6 md:px-10 lg:px-20 -mt-20 sm:-mt-28 relative z-30 sticky top-0 transition-all duration-300">
                        <div className="bg-[#FAF7F2]/80 backdrop-blur-2xl border-2 border-dashed border-gold-muted/40 rounded-[28px] md:rounded-[40px] p-4 md:p-6 shadow-luxury-lg flex flex-col items-center xl:items-start xl:flex-row gap-6 xl:gap-8 transition-all hover:border-gold-muted/60 group">
                            {/* Avatar Container */}
                            <div className="relative shrink-0">
                                <div className="relative group/avatar">
                                    <RankAvatar
                                        ...
                                        size={140}
                                        className={isBirthdayMonth ? "shadow-[0_0_30px_rgba(255,215,0,0.4)] transition-all duration-1000" : ""}
                                    />
                            ...
                            <div className="flex-1 flex flex-col gap-2 w-full">
                                <div className="flex flex-col">
                                    <div className="flex items-center gap-6 justify-center xl:justify-start">
                                        <div className="relative group/name">
                                            <h1
                                                className="font-serif font-black text-text-main tracking-tighter drop-shadow-sm group-hover:text-gold-muted transition-all duration-500 italic pb-2 break-words relative overflow-visible"
                                ...
                                {/* Contact Strip - Consistent Gold Theme - Tightened top spacing - One Line */}
                                <div className="flex flex-nowrap items-center gap-4 pt-2 border-t border-gold-muted/10 overflow-x-auto no-scrollbar">
                                    ...
                                    <div className={`flex items-center gap-3 px-6 py-3 rounded-2xl border transition-all ${isBirthdayMonth ? 'bg-gradient-to-r from-rose-50 to-white border-rose-200' : 'bg-white border-gold-muted/10'} text-gold-muted shadow-sm hover:border-gold-muted/30`}>
                                        <Calendar size={16} strokeWidth={2.5} className={isBirthdayMonth ? 'text-rose-500' : ''} />
                                        <span className={`text-[15px] font-black tracking-tight ${isBirthdayMonth ? 'text-rose-600' : ''}`}>
                                            {formatBirthday(activeCustomer.birthday)}
                                            {isBirthdayMonth && <span className="ml-2 animate-bounce-slow inline-block">🥳</span>}
                                        </span>
                                        {calculateAge(activeCustomer.birthday) && (
                                            <span className={`${isBirthdayMonth ? 'bg-rose-100 text-rose-600 border-rose-200' : 'bg-gold-muted/10 text-gold-muted border-gold-muted/20'} text-[10px] font-black px-3 py-1 rounded-xl ml-1 border`}>
                                    ...
                                    <div className="flex items-center gap-2 text-gold-muted/60">
                                        ...
                                        <span className="text-[11px] font-black uppercase tracking-widest italic">
                                            Lần cuối đến Xinh:  {activeCustomer.lastVisit && activeCustomer.lastVisit !== 'Chưa có' ?
                                                formatBirthday(activeCustomer.lastVisit)
                                                : 'chưa có dữ liệu'}
                                        </span>
                                    </div>
                                </div>
                    ...
                    {/* Navigation Tabs - Included in Fixed Top Section */}
                    <div className="px-6 md:px-10 pb-6">
                        <div className="flex flex-nowrap items-center justify-start md:justify-center gap-1 md:gap-2 p-2 bg-beige-soft/80 rounded-[28px] md:rounded-[32px] border border-gold-light/20 backdrop-blur-xl shadow-luxury overflow-x-auto no-scrollbar">
                            {tabs.map(tab => (
                                <button
                                    key={tab.id}
                                    onClick={() => setActiveTab(tab.id)}
                                    className={`flex flex-row items-center justify-center gap-2 py-3 px-6 md:py-4 md:px-8 rounded-[20px] md:rounded-[24px] font-black uppercase tracking-widest transition-all shrink-0 whitespace-nowrap ${activeTab === tab.id
                                        ? 'bg-text-main text-white shadow-xl scale-[1.05]'
                                        : 'text-text-soft hover:text-gold-muted hover:bg-white'
                                        }`}
                                    style={{ fontSize: 'clamp(9px, 0.8vw, 13px)' }}
                                >
                ...
                <div className="flex-1 overflow-y-auto luxury-scrollbar relative bg-white/40 px-6 md:px-10 lg:px-16 py-8">
                    <style jsx global>{`
                        @keyframes waving-hand { 0%, 100% { transform: rotate(-5deg); } 50% { transform: rotate(15deg); } }
                        @keyframes swinging-icon { 0%, 100% { transform: rotate(-8deg); transform-origin: top center; } 50% { transform: rotate(8deg); transform-origin: top center; } }
                                                ...
                                                <div className="space-y-8 flex-1">
                                                    ...
                                                    <div className="grid grid-cols-2 gap-10">
                                                        ...
                                                        <div className="bg-white/5 p-4 rounded-2xl border border-white/5 col-span-2 text-center mt-[-1rem]">
                                                            ...
                                                            <p className="text-[14px] font-black text-white italic">{activeCustomer.lastVisit && activeCustomer.lastVisit !== 'Chưa có' ? formatBirthday(activeCustomer.lastVisit) : 'Chưa có'}</p>
                                                        </div>
                                                    </div>
                                                </div>
                            ...
                            <div className="grid grid-cols-1 gap-6">
                                {loadingDossier ? (
                                    ...
                                    dossier.serviceOrders.sort((a, b) => b.createdAt.localeCompare(a.createdAt)).map(order => (
                                        <div
                                            key={order.id}
                                            className="bg-white border border-gold-light/20 rounded-[32px] p-6 hover:border-gold-muted/50 transition-all hover:shadow-luxury group cursor-pointer"
                                            ...
                                            <div className="flex flex-col md:flex-row justify-between gap-6">
                                                <div className="space-y-4 flex-1">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <span className={`px-2.5 py-1 rounded-lg text-[8px] font-black uppercase tracking-widest border ${order.status === 'completed' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' :
                                            ...
                                            <tbody className="divide-y divide-gold-light/5">
                                                {loadingDossier ? (
                                                    ...
                                                    dossier.serviceOrders.map(order => (
                                                        <tr
                                                            key={order.id}
                                                            className="hover:bg-beige-soft/10 transition-colors group cursor-pointer"
                                                            onClick={() => setSelectedOrder(order)}
                                                        >
                                                            <td className="px-6 py-4">
                                                    ...
                                                    <div className="flex gap-2 bg-beige-soft/20 p-1.5 rounded-2xl border border-gold-light/10 shadow-inner">
                                                        {(['cash', 'bank', 'wallet'] as const).map(m => (
                                                            <button
                                                                ...
                                                                className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${payMethod === m
                ...
                {/* Service Order Detail Overlay */}
                {selectedOrder && (
                    <div
                        className="fixed inset-0 z-[11000] bg-black/60 backdrop-blur-md flex items-center justify-center p-4 animate-fade-in"
                        onClick={() => setSelectedOrder(null)}
                    >
                        <div
                            className="bg-[#FAF8F6] w-full max-w-2xl rounded-[40px] shadow-3xl overflow-hidden border border-gold-light/20 animate-modal-up flex flex-col max-h-[85vh]"
                            onClick={(e) => e.stopPropagation()}
                        >
                            {/* Header Details */}
                            <div className="p-8 pb-4 flex justify-between items-start">
                                <div>
                                    <div className="flex items-center gap-2 mb-1">
                                        <div className="w-1.5 h-1.5 rounded-full bg-gold-muted animate-pulse"></div>
                                        <span className="text-[10px] font-black uppercase tracking-widest text-gold-muted">Chi tiết phiếu dịch vụ</span>
                                    </div>
                                    <h3 className="text-2xl font-serif font-black text-text-main italic uppercase">{selectedOrder.code}</h3>
                                    <p className="text-[11px] text-text-soft font-bold opacity-60 mt-1">
                                        Ngày lập: {new Date(selectedOrder.createdAt).toLocaleDateString('vi-VN', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
                                    </p>
                                </div>
                                <button
                                    onClick={() => setSelectedOrder(null)}
                                    className="w-12 h-12 rounded-2xl bg-white text-text-soft hover:text-text-main flex items-center justify-center transition-all border border-gold-light/10 shadow-sm active:scale-95"
                                >
                                    <X size={20} />
                                </button>
                            </div>
                            ...
                            {/* Content Scrollable */}
                            <div className="px-8 pb-8 overflow-y-auto luxury-scrollbar flex-1">
                                <div className="space-y-8 mt-6">
                                    {/* Line Items Section */}
                                    <div className="space-y-4">
                                        <p className="text-[10px] font-black text-text-soft uppercase tracking-widest opacity-40 px-2 italic font-serif">Danh mục dịch vụ & sản phẩm</p>
                                        <div className="space-y-3">
                                            {(selectedOrder.lineItems || selectedOrder.line_items || []).map((item: any, i: number) => (
                                                <div key={i} className="bg-white p-5 rounded-3xl border border-gold-light/10 shadow-sm flex justify-between items-center group hover:border-gold-muted/30 transition-all">
                                                    <div className="flex items-center gap-4">
                                                        <div className="w-10 h-10 rounded-xl bg-gold-muted/5 text-gold-muted flex items-center justify-center shrink-0">
                                                            {item.serviceType === 'card' ? <Wallet size={18} /> :
                                                                item.serviceType === 'package' ? <Flower size={18} /> : <Sparkles size={18} />}
                                                        </div>
                                                        <div>
                                                            <p className="text-sm font-black text-text-main group-hover:text-gold-muted transition-colors">{item.serviceName}</p>
                                                            <div className="flex items-center gap-2 mt-0.5">
                                                                <span className="text-[9px] font-black uppercase tracking-widest text-text-soft/40">{item.categoryName || 'Dịch vụ'}</span>
                                                                <span className="text-gray-200">|</span>
                                                                <span className="text-[9px] font-black text-gold-muted uppercase tracking-widest">Qty: {item.quantity || 1}</span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div className="text-right">
                                                        <p className="text-sm font-serif font-black text-text-main italic">{(item.price || 0).toLocaleString()}đ</p>
                                                        {item.discount > 0 && (
                                                            <p className="text-[10px] text-rose-500 font-bold italic">-{item.discount.toLocaleString()}đ</p>
                                                        )}
                                                    </div>
                                                </div>
                                            ))}
                                    ...
                                    <div className="bg-text-main p-8 rounded-[38px] text-white shadow-luxury relative overflow-hidden shrink-0">
                                        <div className="absolute top-0 right-0 w-32 h-32 bg-gold-muted/10 rounded-full translate-x-10 -translate-y-10 blur-2xl"></div>

                                        <div className="space-y-4 relative z-10">
                                            <div className="flex justify-between items-center text-white/40 ">
                                                <span className="text-[10px] font-black uppercase tracking-widest">Tổng tiền phiếu</span>
                                                <span className="font-serif italic">{(selectedOrder.totalAmount || 0).toLocaleString()}đ</span>
                                            </div>
                                            <div className="flex justify-between items-center text-emerald-400">
                                                <span className="text-[10px] font-black uppercase tracking-widest">Đã thanh toán</span>
                                                <span className="text-lg font-serif font-black italic">{(selectedOrder.actualAmount || selectedOrder.actual_amount || 0).toLocaleString()}đ</span>
                                            </div>
                                            {selectedOrder.debtAmount > 0 && (
                                                <div className="flex justify-between items-center text-rose-400 pt-2 border-t border-white/10">
                                                    <span className="text-[10px] font-black uppercase tracking-widest">Còn nợ phiếu</span>
                                                    <span className="text-xl font-serif font-black italic">{(selectedOrder.debtAmount || 0).toLocaleString()}đ</span>
                                                </div>
                                            )}
                                        </div>
                                    </div>

                                    {/* Payments History in Detail */}
                                    {(selectedOrder.payments && selectedOrder.payments.length > 0) && (
                                        <div className="space-y-4 pb-4">
                                            <p className="text-[10px] font-black text-text-soft uppercase tracking-widest opacity-40 px-2 italic font-serif">Lịch sử thanh toán</p>
                                            <div className="space-y-2">
                                                {selectedOrder.payments.map((p: any, idx: number) => (
                                                    <div key={idx} className="flex justify-between items-center p-3 px-5 bg-white rounded-2xl border border-gold-light/5 text-[12px]">
                                                        <div className="flex items-center gap-3">
                                                            <div className="w-1.5 h-1.5 rounded-full bg-emerald-500"></div>
                                                            <span className="text-text-soft font-bold uppercase tracking-tight">{p.method === 'cash' ? 'Tiền mặt' : p.method === 'bank' ? 'Chuyển khoản' : 'Thẻ/Ví'}</span>
                                                            <span className="text-text-soft/40 italic">({new Date(p.date).toLocaleDateString('vi-VN')})</span>
                                                        </div>
                                                        <span className="font-black text-text-main italic">+{(p.amount || 0).toLocaleString()}đ</span>
                                                    </div>
                                                ))}
                                            </div>
                                        </div>
                            ...
                            <div className="p-8 pt-4">
                                <button
                                    onClick={() => setSelectedOrder(null)}
                                    className="w-full py-4 bg-beige-soft text-text-soft hover:text-text-main font-black text-[11px] uppercase tracking-[0.2em] rounded-2xl transition-all active:scale-95 shadow-sm border border-gold-light/10"
                                >
                                    Đóng chi tiết
                                </button>
                            </div>

...
import { AppState, User, Branch, PaymentAccount, Category, MonthlyPlan, Transaction, ActivityLog, RedFlag } from '@/lib/types'
...
export async function fetchAllData(): Promise<AppState | null> {
    try {
        ...
        const batch1 = await Promise.all([
            ...
            fetchAllRows('crm_appointments').then(data => ({ data, error: null })),
        ...
        rawQueries.forEach(q => {
            if (q.res && 'error' in q.res && q.res.error) {
        ...
        const appointmentCustomerIds = resAppointments.map((a: any) => a.customer_id)
        const allTargetCustomerIds = Array.from(new Set([...orderCustomerIds, ...appointmentCustomerIds].filter(Boolean)))

        let orderCustomersMap: Record<string, any> = {}
        if (allTargetCustomerIds.length > 0) {
            const { data: oCusts } = await supabase.from('crm_customers')
                .select('id, full_name, phone, avatar, rank, professional_notes, medical_notes')
                .in('id', allTargetCustomerIds)
            if (oCusts) {
                oCusts.forEach(c => { orderCustomersMap[c.id] = c })
            }
        }
        ...
        return {
            ...
            appointments: resAppointments.map((a: any) => {
                const c = orderCustomersMap[a.customer_id]
                return {
                    ...
                    customerName: c?.full_name || 'Khách vãng lai',
            ...
            leads: resLeads.map((l: any) => ({
                ...
                phoneConfirmed: l.phone_confirmed,
                confirmedAt: l.confirmed_at,
                confirmedBy: l.confirmed_by,
                actualServiceValue: l.actual_service_value,
                serviceNote: l.service_note,
            ...
            serviceOrders: resServiceOrders.map((o: any) => {
                const c = orderCustomersMap[o.customer_id]
                return {
                    id: o.id,
                    code: o.code,
            ...
            serviceCategories: resServiceCategories.map((sc: any) => ({
                id: sc.id,
                name: sc.name,
...
/**
 * Fetches all necessary data for a specific customer to recalculate stats
 */
export async function fetchCustomerHistory(customerId: string) {
    const [orders, appts, customer] = await Promise.all([
        supabase.from('crm_service_orders').select('*').eq('customer_id', customerId),
        supabase.from('crm_appointments').select('*').eq('customer_id', customerId),

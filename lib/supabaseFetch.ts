...
export async function fetchAllData(): Promise<AppState | null> {
    try {
        ...
        // Manual Join: Fetch customers for the service orders since FK relationship is missing
        const orderCustomerIds = Array.from(new Set(resServiceOrders.map((o: any) => o.customer_id).filter(Boolean)))
        let orderCustomersMap: Record<string, any> = {}
        if (orderCustomerIds.length > 0) {
            const { data: oCusts } = await supabase.from('crm_customers')
                .select('id, full_name, phone, avatar')

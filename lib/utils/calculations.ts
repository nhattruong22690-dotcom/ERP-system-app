import { Category, MonthlyPlan, CategoryPlan, CashFlowRow, Transaction, AlertItem, Branch, Lead, CommissionSetting, Tier, Customer, ServiceOrder, Appointment, MembershipTier, AppState, CustomerRank, TreatmentCard } from '@/lib/types'
import { getVNToday } from './date'
...
export function calculateKpiTieredCommissions(
    ...
    // 2. Tìm quy tắc kpi_tiered & Xác định vai trò
    const userObj = state.users.find(u => u.id === userId)
    const userRole = userObj?.title?.toLowerCase() || userObj?.displayName?.toLowerCase() || ""
    ...
    const isConsultant = dept === 'sale' || userRole.includes('tư vấn') || userRole.includes('consultant')
    ...
    // 6. Tính toán chi tiết Bonus theo Quy tắc mới
    const processedLeads = userLeads.map(l => {
        let bonusSdt = 0
        let bonusRevenue = 0

        // Quy tắc Sale Page: 10k/SĐT (confirmed) + 5% doanh thu
        if (isPage) {
            if (l.phoneConfirmed) {
                bonusSdt = tier.bonusWithPayment || 10000 
            }
            if (l.actualServiceValue && l.actualServiceValue > 0) {
                const rate = tier.commissionRate || 5 // Mặc định 5%
                bonusRevenue = Math.round(l.actualServiceValue * (rate / 100))
            }
        }
        return { ...l, bonusSdt, bonusRevenue }
    })
    ...
    const processedAppointments = userAppointments.map(a => {
        ...
        // Quy tắc Sale Tele: Booking (10-25k), Check-in (25-30k), Service (30k + 5%)
        if (isTele) {
            const hasService = a.status === 'completed' || (a.price && a.price > 0)
            const hasCheckin = a.status === 'arrived' || a.status === 'completed'

            if (hasService) {
                // Khách đến + phát sinh dịch vụ
                bonusCheckin = tier.bonusService || 30000
                const rate = tier.commissionRate || 5
                bonusPsDv = Math.round((a.price || 0) * (rate / 100))
            } else if (hasCheckin) {
                // Khách đến + không phát sinh
                bonusCheckin = tier.bonusNoPayment || 25000
            } else if (a.status === 'confirmed' || a.status === 'booked' || a.status === 'pending') {
                // Chỉ đặt lịch
                bonusCheckin = tier.bonusBooking || 10000
            }
        }

        // Quy tắc Tư vấn: Doanh thu tính 100% vào KPI (đã tính ở actualKpi phần Target)
        if (isConsultant && a.status === 'completed') {
             const rate = tier.commissionRate || 0
             bonusPsDv = Math.round((a.price || 0) * (rate / 100))
        }
    ...
    if (isTele) {
        ...
        if (bonusCheckinTotal > 0) details.push(`Thưởng Check-in/Booking: +${fmtVND(bonusCheckinTotal)}`)
    } else if (isConsultant) {
        commissionAmount = processedAppointments.reduce((s, a) => s + (a.bonusPsDv || 0), 0)
        if (commissionAmount > 0) details.push(`Hoa hồng tư vấn: +${fmtVND(commissionAmount)}`)
    } else if (isPage) {
        // Sale Page
        const bonusSdtTotal = processedLeads.reduce((s, l) => s + (l.bonusSdt || 0), 0)
        const bonusRevTotal = processedLeads.reduce((s, l) => s + (l.bonusRevenue || 0), 0)
        bonusKpi += bonusSdtTotal
        commissionAmount = bonusRevTotal
        if (bonusSdtTotal > 0) details.push(`Thưởng Lấy SĐT: +${fmtVND(bonusSdtTotal)}`)
        if (bonusRevTotal > 0) details.push(`Hoa hồng chuyển đổi: +${fmtVND(bonusRevTotal)}`)
    }

    // Thưởng cố định nếu có trong tier
    if (tier.bonusAmount && tier.bonusAmount > 0) {
        bonusKpi += tier.bonusAmount
        details.push(`Thưởng mốc KPI: +${fmtVND(tier.bonusAmount)}`)
    }
...
/**
 * V23: Tính KPI cho Kỹ thuật viên & Bác sĩ dựa trên hoa hồng thực hiện dịch vụ
 */
export function calculateStaffServiceKpi(
    userId: string,
    monthStr: string,
    serviceOrders: ServiceOrder[]
): { totalCommission: number; orderCount: number; details: string[] } {
    const details: string[] = []
    let totalCommission = 0
    let orderCount = 0

    // Lọc các phiếu hoàn thành trong tháng
    const myOrders = (serviceOrders || []).filter(o => 
        o.status === 'completed' && 
        o.createdAt.startsWith(monthStr)
    )

    myOrders.forEach(order => {
        let orderStaffCommission = 0
        const itemsWithMe: string[] = []

        order.lineItems.forEach(li => {
            const mySplit = li.staffSplits?.find(sp => sp.staffId === userId)
            if (mySplit) {
                orderStaffCommission += mySplit.amount
                itemsWithMe.push(`${li.serviceName} (${fmtVND(mySplit.amount)})`)
            }
        })

        if (orderStaffCommission > 0) {
            totalCommission += orderStaffCommission
            orderCount++
            details.push(`Phiếu ${order.code}: +${fmtVND(orderStaffCommission)} [${itemsWithMe.join(', ')}]`)
        }
    })

    return { totalCommission, orderCount, details }
}
